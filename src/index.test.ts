...
describe('API Routes', () => {
  ...
  describe('POST /todos', () => {
    it('should create and return a new todo', async () => {
      const newTodo = { id: 'some-id', objective: 'Work', title: 'Task 2', completed: false };
  ...
  describe('PUT /todos/:id', () => {
    it('should update and return the todo', async () => {
      const updatedTodo = { id: '1', objective: 'Work', title: 'Task 1 Updated', completed: true };
      vi.mocked(dbRepo.updateTodo).mockResolvedValue(updatedTodo);

      const res = await app.request('/todos/1', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ objective: 'Work', title: 'Task 1 Updated', completed: true })
      });

      expect(res.status).toBe(200);
      expect(await res.json()).toEqual(updatedTodo);
      expect(dbRepo.updateTodo).toHaveBeenCalledWith('1', 'Work', 'Task 1 Updated', true);
    });
  ...
  describe('DELETE /todos/:id', () => {
    ...
    it('should return 400 if objective query is missing', async () => {
      ...
      expect(await res.json()).toEqual({ error: 'Missing objective query parameter' });
    });
