...
export class CosmosRepo {
  ...
  private getContainer() {
    ...
    if (!databaseName || !containerName) throw new Error("COSMOS_DATABASE and COSMOS_CONTAINER must be set");
  ...
  async listObjectives(): Promise<string[]> {
    ...
    const { resources: objectives } = await container.items.query(querySpec).fetchAll();
  ...
  async updateTodo(id: string, objective: string, title: string, completed: boolean): Promise<Todo | null> {
    const container = this.getContainer();
