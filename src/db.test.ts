...
describe('CosmosRepo', () => {
  ...
  describe('Initialization', () => {
    it('should throw if COSMOS_ENDPOINT is missing', () => {
      ...
      expect(() => new CosmosRepo()).toThrowError('COSMOS_ENDPOINT must be set');
    });
  ...
  describe('listObjectives', () => {
    it('should throw if DB/Container env vars are missing', async () => {
      ...
      const repo = new CosmosRepo();
      await expect(repo.listObjectives()).rejects.toThrowError('COSMOS_DATABASE and COSMOS_CONTAINER must be set');
    });

    it('should return objectives', async () => {
      mockItemsQueryFetchAll.mockResolvedValueOnce({ resources: ['Work', 'Personal'] });
      const repo = new CosmosRepo();
      const res = await repo.listObjectives();
      expect(res).toEqual(['Work', 'Personal']);
      expect(mockQuery).toHaveBeenCalledWith({ query: "SELECT DISTINCT VALUE c.objective FROM c" });
    });
  });

  describe('listTodos', () => {
    it('should return todos', async () => {
      const mockTodos = [{ id: '1', title: 'Test', objective: 'Work', completed: false }];
  ...
  describe('createTodo', () => {
    it('should create and return todo', async () => {
      const newTodo = { id: '1', title: 'Test', objective: 'Work', completed: false };
    ...
    it('should throw if resource creation fails', async () => {
      ...
      const repo = new CosmosRepo();
      await expect(repo.createTodo({ id: '1', title: 'T', objective: 'O', completed: false })).rejects.toThrowError('Failed to create todo resource');
    });
  });

  describe('updateTodo', () => {
    it('should update and return todo', async () => {
      const oldTodo = { id: '1', title: 'Old', objective: 'Work', completed: false };
      const newTodo = { id: '1', title: 'New', objective: 'Work', completed: true };
      ...
      expect(mockItemReplace).toHaveBeenCalledWith({ ...oldTodo, title: 'New', completed: true });
    });
    ...
    it('should rethrow if item read throws other error', async () => {
      ...
      mockItemRead.mockRejectedValueOnce(error);
      const repo = new CosmosRepo();
      await expect(repo.updateTodo('1', 'Work', 'New', true)).rejects.toThrowError('DB Error');
    });
  });
  ...
  describe('deleteTodo', () => {
    ...
    it('should rethrow if item delete throws other error', async () => {
      ...
      mockItemDelete.mockRejectedValueOnce(error);
      const repo = new CosmosRepo();
      await expect(repo.deleteTodo('1', 'Work')).rejects.toThrowError('DB Error');
    });
  });
});
